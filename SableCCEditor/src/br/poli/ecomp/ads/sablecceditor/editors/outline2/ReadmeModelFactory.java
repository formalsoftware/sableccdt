...
public class ReadmeModelFactory {
    ...
    private LinkedList<String> listaLexers = new LinkedList<String>();
    ...
    public AdaptableList getContentOutline(IAdaptable adaptable, IDocument document) {
        return new AdaptableList(getToc((IFile) adaptable, document));
    }
     ...
     */
    public AdaptableList getSections(IFile file, IDocument document) {
        MarkElement[] topLevel = getToc(file,document);
     ...
     */
    private MarkElement[] getToc(IFile file, IDocument document) {
    	if (registryLoaded == false)
            loadParser();
    		SableCCFileParser sableParser = new SableCCFileParser();
    		return sableParser.parseFile(file,document);
    }
